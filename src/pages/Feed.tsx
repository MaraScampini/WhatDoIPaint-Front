            ...
            {userProjects.length > 0 ? (
                <div className='flex ms-10 space-x-5'>
                    {userProjects.map((project) => (
                        <div
                            key={project.id}
                            className='w-1/4 aspect-square bg-cover bg-center rounded-xl relative transition-all duration-300 ease-in-out hover:outline hover:outline-lightTeal hover:outline-1 hover:cursor-pointer'
                            style={{ backgroundImage: `url(${project.image})` }}
                        >
                            <button onClick={togglePriority} className='text-lightTeal absolute right-3 top-3'>
                                {project.priority ? (
